...
// src/cli-plan-checklist.ts
var import_fs = __toESM(require("fs"), 1);
var import_path = __toESM(require("path"), 1);
var import_os = __toESM(require("os"), 1);
var CONFIG_DIR = import_path.default.join(import_os.default.homedir(), ".plan-checklist");
...
function saveConfig(config2) {
  try {
    ...
    import_fs.default.writeFileSync(CONFIG_FILE, JSON.stringify(newConfig, null, 2));
  } catch (e) {
    console.error(`Warning: Failed to save config: ${e.message}`);
  }
}
var config = loadConfig();
var getBaseUrl = (serverOption) => {
  return serverOption || config.server || process.env.CHECKLIST_API_URL || "http://localhost:8585";
};
var getNamespace = (namespaceOption) => {
  return namespaceOption || config.namespace || "default";
};
var program2 = new Command();
program2.name("plan-checklist").description("CLI tool to interact with Checklist MCP Server to manage tasks").version("1.0.0").option("-s, --server <url>", "Override the default server URL and save it").option("-n, --namespace <ns>", "Namespace isolation and save it").hook("preAction", (thisCommand) => {
...
program2.action(async () => {
  ...
  console.log(`Current Server: ${getBaseUrl(program2.opts().server)}`);
  console.log(`Current Namespace: ${getNamespace(program2.opts().namespace)}`);
  console.log("Checking server health...");
  try {
    const url = `${getBaseUrl(program2.opts().server)}/health`;
...
async function fetchGet(url, errorMessage) {
  try {
    const res = await fetch(url);
    const data = await res.json();
    if (!res.ok) {
...
async function fetchPost(url, body, errorMessage) {
  try {
    const res = await fetch(url, {
      method: "POST",
...
program2.command("list").description("Retrieve all tasks for a session").argument("<sessionId>", "The session ID").action(async (sessionId, options, command) => {
...
program2.command("update").description("Update tasks at a specific path").argument("<sessionId>", "The session ID").option("-p, --path <path>", "The hierarchical path", "/").option("-f, --file <tasks.json>", "JSON file containing tasks array").option("-t, --text <tasks_json_string>", "JSON string containing tasks array").action(async (sessionId, options, command) => {
  let tasksData = "";
  if (options.file) {
    try {
      tasksData = import_fs.default.readFileSync(import_path.default.resolve(options.file), "utf8");
    } catch (e) {
    ...
    try {
      tasksData = import_fs.default.readFileSync(process.stdin.fd, "utf-8");
    } catch (e) {
      if (e.code === "EAGAIN") {
...
program2.command("done").description("Mark one or more tasks as done").argument("<sessionId>", "The session ID").argument("<taskId...>", "One or more task IDs to mark as done").action(async (sessionId, taskIds, options, command) => {
  const opts = command.optsWithGlobals();
  const serverUrl = getBaseUrl(opts.server);
  const namespace = getNamespace(opts.namespace);
