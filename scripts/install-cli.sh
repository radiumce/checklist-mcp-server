...
# Usage: curl -fsSL https://raw.githubusercontent.com/radiumce/checklist-mcp-server/main/scripts/install-cli.sh | bash
...
DOWNLOAD_URL="https://raw.githubusercontent.com/radiumce/checklist-mcp-server/main/bin/plan-checklist.cjs"
...
if [ "$1" = "--local" ] || [ -f "./bin/plan-checklist.cjs" ]; then
    echo "Installing from local bin/plan-checklist.cjs..."
    if [ ! -f "bin/plan-checklist.cjs" ]; then
        echo "Error: bin/plan-checklist.cjs not found. Please run 'npm run build:cli' first."
        exit 1
    fi
    cp bin/plan-checklist.cjs "$EXECUTABLE_PATH"
else
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        if ! curl -fsSL "$DOWNLOAD_URL" -o "$EXECUTABLE_PATH"; then
            echo "Error: Failed to download the executable. Make sure the 'bin/plan-checklist.cjs' is pushed to the repository main branch."
            exit 1
        fi
